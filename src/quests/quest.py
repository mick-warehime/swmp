...
class Quest2(object):
    """An object used to organize a sequence scenes quest. A quest is
    represented as a directed graph (DiGraph). Clients should only ask a quest
     for the next dungeon and the quest object will keep track of the internal
     state of the quest."""
    ...
    def _root_scene(self) -> Any:
        ...
        if len(root) == 1:
            raise ValueError(
                'graph should only have 1 root, found {}'.format(root))
